...
public class Accueil extends JPanel {
	...
	/**
	 * Sur le double-clic sur une mission en retard, on affiche une popup pour rapidement mettre la date de fin réelle
	 */
	public void PopUpMissionEnRetard() {
		...
		if (n == JOptionPane.YES_OPTION) {
			...
			if (dp.getDate() == null) {
				JOptionPane.showMessageDialog(new JFrame(), "La date de fin réelle est invalide.", "Date invalide",
				...
				if (DateDeDebut.compareTo(DateDeFin) > 0) {
					JOptionPane.showMessageDialog(new JFrame(),
							"La date de fin doit être supérieure ou égale à la date de début.", "Date invalide",
							JOptionPane.WARNING_MESSAGE);
					PopUpMissionEnRetard();
