...
public class Test {
	public static void main(String[] args) throws CSVException, ParseException {
		...
		CSVObjects<Competence> competences_csv = new CSVObjects<>(Competence.class, cache);
		...
		for (Entry<String, Competence> entry : cache.getCache(Competence.class))
		...
		CSVObjects<Employee> employes_csv = new CSVObjects<>(Employee.class, cache);
		...
		System.out.println(cache.getCache(Employee.class).get(genereated_id));
		...
		CSVObjects<CompetenceRequirement> compreq_csv = new CSVObjects<>(CompetenceRequirement.class, cache);
		compreq_csv.addMany(unsaved_objects);
		
		CSVObjects<Mission> missions_csv = new CSVObjects<>(Mission.class, cache);
