...
public class Test {
	public static void main(String[] args) throws CSVException, ParseException {
		...
		/*
		 * for (Entry<String, Competence> entry :
		 * config.getCSVCache().getCache(Competence.class))
		 * System.out.println("key: " + entry.getKey() + " - val: " +
		 * entry.getValue()); // Valeurs cachées competences_csv.getAll(); for
		 * (Entry<String, Competence> entry :
		 * config.getCSVCache().getCache(Competence.class))
		 * System.out.println("key: " + entry.getKey() + " - val: " +
		 * entry.getValue());
		 */
